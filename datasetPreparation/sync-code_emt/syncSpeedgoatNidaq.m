...
% Handle edge cases 
% The first two days of recording used a high sampling rate, can downsample this
if FsNI == 1000000
...
%% Decode sync signal 
%
% find the rising edge of the beginning of the 10Hz sync pulses, which
% define the start of a sync block, and convert these into Speedgoat time
% (ms since the start of model execution)
...
% find midpiont times in between sync times as time bin histogram edges
histrialEdges = mean([syncTimes(1:end-1),syncTimes(2:end)],2);
histrialEdges = [0; histrialEdges; dataDur]; % s

% calculate which synchronization bin each NIDAQ timestamp falls into
[~, timeBins] = histc(Times.nidaq, histrialEdges);
...
rmvIdx = [];
ii = 1;
while timeBins(ii) < 2  % changed from 1 on 2023-08-16 since 
    rmvIdx = [rmvIdx, ii];
    ii = ii + 1;
end
ii = length(timeBins);
lenSyncTimes = length(syncTimes);   % EMT 2023-08-16: should length(syncTimes) be calculated before correupt blocks removed?)
...
if ~isempty(P.Results.saveTags)
    goodTrials = goodTrials & ismember(T.saveTag, P.Results.saveTags);
end
T = T(goodTrials,:);
...
if ismember('simTime', fieldnames(T))
    trialEdges = [cellfun(@(t) t(1), T.simTime), cellfun(@(t) t(end), T.simTime)];
else
    trialEdges = [cellfun(@(t) t(1), T.t), cellfun(@(t) t(end), T.t)];
end
trialEdges = roundn(trialEdges, -3);

% find the first "good" trial (i.e. the first trial when Speedgoat and
% nidaq are running simultaneously)
ii_tEd = 1;
jj_tEd = 1;
while trialEdges(ii_tEd, jj_tEd) < Times.speedgoat(1)
...
trialEdgeIdx = zeros(size(trialEdges));

while ii_tSg <= nTimeSteps && ii_tEd <= nTrials
    
    if Times.speedgoat(ii_tSg) >= trialEdges(ii_tEd,jj_tEd)
        trialEdgeIdx(ii_tEd,jj_tEd) = ii_tSg;
...
if ~P.Results.legacy
    trialEdgeIdx = trialEdgeIdx - round(0.1 * FsNI);
end

% convert data to cell
trialEdgeIdx = mat2cell(trialEdgeIdx, ones(1,nTrials), 2);
...
% newT.trialTime = cellfun(@(tei) Times.nidaq(max(1,tei(1)):max(1,tei(2))), trialEdgeIdx, 'uni', false);
...
% newT.dt = cellfun(@(tSg,tIdxBr) diff(tSg)/diff(tIdxBr), trialEdges, trialEdgeIdx, 'uni', false);
...
%     
% %     newT.emgBR(goodIdx) = cellfun(@(i) rawData(dataCh, i(1):i(2)), trialEdgeIdx(goodIdx), 'uni', false);
%     newT.emgBR(goodIdx) = cellfun(@(i,t) rawData(dataChan, i(1)+(0:length(t)-1)), trialEdgeIdx(goodIdx), newT.trialTime(goodIdx), 'uni', false);
%     
%     % vertically align
...
%         trIdx = trialEdgeIdx{iTr}(1):trialEdgeIdx{iTr}(2);
%         
...
if ~isempty(P.Results.spikes)
    ...
    for iTr = firstGoodTrial:nTrials
        ...
        trialEdgesNidaq = Times.nidaq(trialEdgeIdx{iTr});
        
        for jUn = 1:nUnits
            % find spikes that fall within trial bounds
            trialSpikes = tSpk{jUn}(tSpk{jUn} >= trialEdgesNidaq(1) & tSpk{jUn} <= trialEdgesNidaq(2));
            
            % record spike times in T table
            newT.tSpk{iTr,jUn} = trialSpikes - trialEdgesNidaq(1);
        end
    end
end
