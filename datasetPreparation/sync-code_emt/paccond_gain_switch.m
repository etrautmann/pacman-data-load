...
function [Task, nGlitch, AlignStats, alignIndices, condID] = paccond_gain_switch(T, neuralRecType, varargin)
%% Parse inputs
...
%         T.forceFilt(condIdx), alignIdx, T.trialParams(condIdx), 'uni', false);
    ...
    Task.cursorPosition{ii} = cell2mat(cellfun(@(cp, ai) cp(ai+trIdxSG), T.cursorPosition(condIdx), alignIdx, 'uni', false))';
